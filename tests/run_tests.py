...
def add_to_python_path(location_name, folder):
    print "Adding %s location to python_path: %s" % (location_name, folder)
    sys.path = [folder] + sys.path

add_to_python_path(
    "tank",
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "python", "tk-core", "python"))
)
add_to_python_path(
    "shotgun_desktop",
